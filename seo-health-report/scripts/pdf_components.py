...
def create_cover_page(
    ...
    company_style = ParagraphStyle(
        "CompanyName", parent=styles["Heading1"], fontSize=20, alignment=TA_CENTER
    )
...
def create_score_gauge(
    score: int, label: str, brand_colors: Optional[Dict[str, str]] = None
) -> Drawing:
...
def create_findings_table(
    findings: List[str], brand_colors: Optional[Dict[str, str]] = None
) -> Table:
...
def create_recommendations_list(
    recommendations: List[Dict[str, Any]], brand_colors: Optional[Dict[str, str]] = None
) -> List:
    ...
    for i, rec in enumerate(recommendations[:10], 1):
        ...
        rec_style = ParagraphStyle(
            f"Rec{i}", parent=styles["Normal"], fontSize=10, leftIndent=20, spaceAfter=8
        )
...
def create_issues_table(
    issues: List[Dict[str, Any]], brand_colors: Optional[Dict[str, str]] = None
) -> Table:
