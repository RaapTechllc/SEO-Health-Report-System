...
try:
    from seo_health_report.scripts.cache import TTL_HTTP_FETCH, cached
...
def fetch_url(url: str, timeout: int = None) -> Optional[str]:
    ...
    """
    if timeout is None:
        timeout = _config.crawl_timeout

    try:
        import requests

        headers = {"User-Agent": _config.crawl_user_agent}
...
def check_sitemaps(
    ...
    result = {"sitemaps_found": [], "total_urls": 0, "issues": [], "score": 0}
...
def check_redirects(url: str, max_chain: int = 5) -> dict[str, Any]:
    ...
    """
    result = {"final_url": url, "chain": [], "chain_length": 0, "issues": []}
    ...
    try:
        ...
        for _ in range(max_chain):
            response = requests.head(
                current_url, headers=headers, allow_redirects=False, timeout=10
            )
...
def analyze_meta_robots(html: str, url: str) -> dict[str, Any]:
    ...
    """
    result = {"directives": [], "indexable": True, "followable": True, "issues": []}
...
def analyze_internal_links(html: str, page_url: str, base_url: str) -> dict[str, Any]:
    ...
    for href, _anchor_text in matches:
...
def analyze_crawlability(
    ...
    if sitemaps_result.get("sitemaps_found"):
        result["findings"].append(
            f"Found {len(sitemaps_result['sitemaps_found'])} sitemap(s) with {sitemaps_result['total_urls']} URLs"
        )
    else:
    ...
    if redirects_result.get("chain_length", 0) > 1:
        result["findings"].append(
            f"Homepage has {redirects_result['chain_length']}-hop redirect chain"
        )
    else:
