                ...
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    if not_after:
                        ...
                        result["findings"].append(
                            f"SSL certificate valid until {expiry.strftime('%Y-%m-%d')}"
                        )
...
def analyze_security_headers(url: str) -> dict[str, Any]:
    ...
    """
    result = {"headers_found": {}, "headers_missing": [], "issues": [], "findings": [], "score": 0}

    try:
        import requests

        headers = {"User-Agent": "SEO-Health-Report-Bot/1.0 (Security Audit)"}
...
def check_mixed_content(url: str) -> dict[str, Any]:
    ...
    """
    result = {"has_mixed_content": False, "mixed_resources": [], "issues": [], "findings": []}
    ...
    try:
        ...
        for pattern, resource_type in http_patterns:
            ...
            for match in matches:
                ...
                if url_match:
                    ...
                    result["mixed_resources"].append({"url": resource_url, "type": resource_type})
        ...
        if result["mixed_resources"]:
            ...
            result["findings"].append(
                f"Mixed content warning: {len(result['mixed_resources'])} HTTP resources found"
            )
        else:
