...
New code should use `packages.config` instead:
...
def get_config(warn_deprecated: bool = True) -> Config:
    """
    Get or create global configuration instance.

    DEPRECATED: Use `from packages.config import get_settings` instead.
    This function is maintained for backward compatibility.

    Args:
        warn_deprecated: If True, emit a deprecation warning (default: True)
    ...
    if warn_deprecated:
        warnings.warn(
            "get_config() is deprecated. Use 'from packages.config import get_settings' instead. "
            "See packages/config/__init__.py for the new configuration API.",
            DeprecationWarning,
            stacklevel=2,
        )
