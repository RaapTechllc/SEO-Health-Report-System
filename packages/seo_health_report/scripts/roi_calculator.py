...
class ROIProjection:
    ...
    def roi_percentage(self) -> float:
        if self.implementation_cost == 0:
            return float("inf")
        return (
            (self.annual_revenue_increase - self.implementation_cost) / self.implementation_cost
        ) * 100
...
def calculate_seo_roi(
    audit_results: dict[str, Any], business_metrics: dict[str, Any]
) -> list[ROIProjection]:
    """
    ...
    if technical.get("score", 0) < 80:
        projections.extend(
            _calculate_technical_roi(technical, monthly_traffic, revenue_per_visitor)
        )
...
def _calculate_technical_roi(
    technical: dict, monthly_traffic: int, revenue_per_visitor: float
) -> list[ROIProjection]:
...
def _calculate_content_roi(
    content: dict, monthly_traffic: int, revenue_per_visitor: float
) -> list[ROIProjection]:
...
def _calculate_ai_roi(
    ai: dict, monthly_traffic: int, revenue_per_visitor: float
) -> list[ROIProjection]:
...
def generate_roi_summary(projections: list[ROIProjection]) -> dict[str, Any]:
    ...
    overall_roi = (
        ((total_annual_revenue - total_investment) / total_investment * 100)
    ...
    avg_payback_months = (
        total_investment / total_monthly_revenue if total_monthly_revenue > 0 else 0
    )
