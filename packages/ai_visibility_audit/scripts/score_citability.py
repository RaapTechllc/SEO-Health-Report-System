...
def check_original_research(html: str, url: str) -> list[CitableContent]:
    ...
    research_patterns = [
        (
            r"(?:our|we|this)\s+(?:study|research|analysis|survey|report)\s+(?:found|shows|reveals|indicates)",
            "research",
        ),
        (
            r"(?:surveyed|analyzed|studied)\s+\d+[,\d]*\s+(?:people|users|customers|companies|respondents)",
            "survey",
        ),
    ...
    stat_patterns = [
        (
            r"\d+(?:\.\d+)?%\s+of\s+(?:\w+\s+){1,3}(?:said|reported|indicated|prefer|use)",
            "statistic",
        ),
...
def check_tools_and_resources(html: str, url: str) -> list[CitableContent]:
    ...
    tool_patterns = [
        (
            r"<(?:form|input|select|button)[^>]*(?:calculator|tool|checker|analyzer|generator)",
            "tool",
        ),
...
def analyze_site_citability(
    target_url: str, additional_pages: Optional[list[str]] = None
) -> dict[str, Any]:
    """
    ...
    for page_url in pages_to_check:
        ...
        all_citable.extend([CitableContent(**c) for c in result.get("citable_content", [])])
    ...
    return {
        ...
        "all_citable_content": [
            {"content_type": c.content_type, "title": c.title, "url": c.url, "score": c.score}
