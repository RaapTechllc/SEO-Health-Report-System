...
def analyze_core_web_vitals(psi_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    psi_score = psi_data.get("score", 0)
...
def check_resource_optimization(psi_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if result["opportunities"]:
        result["findings"].append(f"Found {len(result['opportunities'])} optimization opportunities")
        result["findings"].append(f"Potential savings: {result['total_savings_ms']}ms, {format_bytes(result['total_savings_bytes'])}")
    else:
...
def analyze_speed(url: str, strategy: str = "mobile") -> Dict[str, Any]:
    ...
    psi_data = get_pagespeed_insights(url, strategy)
