...
def calculate_roi_projection(
    ...
    ai_score = audit_data.get("audits", {}).get("ai_visibility", {}).get("score", 50) or 50
    ...
    if market_intel:
        ...
        ai_rank = benchmark.get("ai_visibility_rank", 5)
        ai_gap = benchmark.get("ai_visibility_gap_to_leader", 20)
    ...
    missed_visits = potential_visits - current_visits
...
**Investment ROI:** A typical engagement ({projection.engagement_cost_range}) delivers an estimated {projection.estimated_roi_multiple} return within the first year, with payback typically achieved within {projection.payback_period_months} months."""
