...
async def generate_html_report_simple(
    ...
    """Generate HTML report using Jinja2 template with fallback."""
    reports_dir = Path("reports") / tenant_id
    reports_dir.mkdir(parents=True, exist_ok=True)
    report_path = reports_dir / f"{audit_result.audit_id}.html"

    try:
        return await _generate_html_report_jinja2(audit_result, raw_result, tenant_id, report_path)
    except Exception as e:
        logger.warning(f"Jinja2 report generation failed, using fallback: {e}")
        return await _generate_html_report_fallback(audit_result, raw_result, report_path)


async def _generate_html_report_jinja2(
    audit_result: AuditResult,
    raw_result: dict,
    tenant_id: str,
    report_path: Path,
    ...
    template_dir = Path(__file__).parent.parent.parent.parent / "packages" / "seo_health_report" / "templates"
    env = Environment(loader=FileSystemLoader(str(template_dir)), autoescape=True)
    template = env.get_template("report.html")

    grade_value = audit_result.grade.value if hasattr(audit_result.grade, "value") else str(audit_result.grade)
    overall_score = audit_result.overall_score or 0
    ...
    # Compute component score colors and percentages
    component_scores = {}
    ...
    for name, data in audits.items():
        ...
        if pct >= 80:
            ...
            color = "#ef4444"
        component_scores[name] = {"score": score, "pct": pct, "color": color}

    # Collect all issues from all components across all audits
    all_components = {}
    for audit_name, audit_data in audits.items():
        for comp_name, comp_data in audit_data.get("components", {}).items():
            key = f"{audit_name}_{comp_name}"
            all_components[key] = comp_data

    # Get branding
    branding = raw_result.get("branding", {})

    context = {
        "company_name": audit_result.company_name,
        "url": audit_result.url,
        "timestamp": audit_result.completed_at or datetime.utcnow().isoformat(),
        "overall_score": overall_score,
        "grade": grade_value,
        "score_class": score_class,
        "grade_description": grade_descriptions.get(grade_value, ""),
        "component_scores": component_scores,
        "components": all_components,
        "recommendations": raw_result.get("audits", {}).get("ai_visibility", {}).get("recommendations", [])
            + raw_result.get("audits", {}).get("content", {}).get("recommendations", [])
            + raw_result.get("audits", {}).get("technical", {}).get("recommendations", []),
        "competitor_benchmarks": raw_result.get("market_intelligence", {}).get("competitor_benchmarks", []),
        "roi_projection": raw_result.get("roi_projection"),
        "primary_color": branding.get("primaryColor", "#1E3A8A"),
        "secondary_color": branding.get("secondaryColor", "#3B82F6"),
        "logo_url": branding.get("logoUrl", ""),
        "footer_text": branding.get("footerText", "Generated by SEO Health Report System"),
    }
...
async def _generate_html_report_fallback(
    audit_result: AuditResult,
    raw_result: dict,
    report_path: Path,
) -> str:
    """Fallback HTML report generation (inline template)."""
    grade_value = (
