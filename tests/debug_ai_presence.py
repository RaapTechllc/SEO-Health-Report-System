...
spec = importlib.util.spec_from_file_location(
    "ai_visibility_audit", os.path.join(ai_vis_path, "__init__.py")
)
...
async def test_ai_presence():
    ...
    output.append(
        f"Google/Gemini Key Present: {bool(os.environ.get('GOOGLE_API_KEY') or os.environ.get('GEMINI_API_KEY'))}"
    )
    output.append("-" * 30)
    ...
    try:
        ...
        output.append(
            f"Claude Result: Mentioned={res.brand_mentioned}, Error={res.error}, ResponseLen={len(res.response)}"
        )
        if res.error:
    ...
    try:
        ...
        output.append(
            f"OpenAI Result: Mentioned={res.brand_mentioned}, Error={res.error}, ResponseLen={len(res.response)}"
        )
        if res.error:
    ...
    try:
        ...
        output.append(
            f"Gemini Result: Mentioned={res.brand_mentioned}, Error={res.error}, ResponseLen={len(res.response)}"
        )
        if res.error:
