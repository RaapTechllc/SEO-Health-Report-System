...
async def benchmark_against_competitors(
    ...
    # If client is rank 1 of 11: (11-1)/11 = 90th percentile
    # If client is rank 11 of 11: (11-11)/11 = 0th percentile -> Clamp to 1st
    raw_percentile = int(100 * (len(all_scores) - rank) / len(all_scores)) if all_scores else 50
    percentile = max(1, raw_percentile)
...
- Percentile: {ordinal(benchmark_report.market_position_percentile)} (Top {100 - benchmark_report.market_position_percentile}%)
...
"""
    ...
    prompt = f"""You are a dedicated Senior SEO Strategist acting as a trusted advisor to the client's executive team (CEO/CMO).
Your goal is to write a high-impact Executive Summary for their Competitive Market Analysis.

The tone must be:
- Strategic & Insightful (focus on business impact, not just metrics)
- Direct & Honest (don't sugarcoat gaps, but offer solutions)
- Human & Conversational (avoid robotic or repetitive phrasing)
- Authoritative yet Accessible (use plain English, not jargon)

STRUCTURE:
1. MARKET POSITION: Start with a clear "tough love" or "congratulatory" statement about where they stand.
2. COMPETITIVE REALITY: Who is winning and why? (Name drops specific competitors).
3. AI THREAT/OPPORTUNITY: Explain why their AI visibility score matters for future revenue.
4. THE WAY FORWARD: 3 specific, high-priority actions.
5. FINAL WORD: A punchy closing sentence on the cost of inaction vs. value of action.

CONTEXT:
{context}

RULES:
- Do NOT use phrases like "In conclusion", "To summarize", or "Delving into".
- Do NOT use "Firstly", "Secondly", "Thirdly".
- Write in short, punchy paragraphs.
- Treat the "0th percentile" or low rankings as a "Critical wake-up call".
- If they are #1, frame it as "Defending the throne".
- THIS IS THE MOST IMPORTANT PART OF THE REPORT. MAKE IT COUNT.
...
Write the executive summary now:"""
    ...
    try:
        ...
        return clean_ai_copy(raw_summary)
