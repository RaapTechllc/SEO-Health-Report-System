...
class CompetitiveAnalyzer:
    ...
    def analyze_competitive_landscape(
        self, prospect_url: str, competitor_urls: list[str]
    ) -> CompetitiveAnalysis:
        ...
        try:
            ...
            try:
                ...
                result = generate_report(
                    target_url=url, company_name="Competitive Analysis", output_format="json"
                )
                return result
            except ImportError:
    ...
    def _build_comparison_matrix(
        self, prospect_report: dict, competitor_reports: dict
    ) -> ComparisonMatrix:
        ...
        competitor_scores = {
            url: report.get("overall_score", 0) for url, report in competitor_reports.items()
        }
        ...
        # Grade comparison
        grade_comparison = {prospect_report["url"]: prospect_report.get("grade", "F")}
        grade_comparison.update(
            {url: report.get("grade", "F") for url, report in competitor_reports.items()}
        )
    ...
    def _identify_ai_visibility_gaps(
        self, prospect_report: dict, competitor_reports: dict
    ) -> list[str]:
    ...
    def _identify_key_differentiators(self, matrix: ComparisonMatrix) -> list[str]:
        ...
        competitor_tech_scores = [
            score for url, score in matrix.technical_comparison.items() if url != prospect_url
        ]
        ...
        competitor_content_scores = [
            score for url, score in matrix.content_comparison.items() if url != prospect_url
        ]
        ...
        competitor_ai_scores = [
            score for url, score in matrix.ai_visibility_comparison.items() if url != prospect_url
        ]
    ...
    def _generate_recommendations(self, matrix: ComparisonMatrix, ai_gaps: list[str]) -> list[str]:
        ...
        if ai_gaps:
            recommendations.append(
                "Implement AI search optimization strategy to capture ChatGPT/Claude traffic"
            )
        ...
        if matrix.prospect_score < 70:
            ...
            recommendations.append(
                "Focus on high-impact improvements to gain competitive advantage"
            )
        else:
            recommendations.append(
                "Maintain leadership position while monitoring competitor movements"
            )
