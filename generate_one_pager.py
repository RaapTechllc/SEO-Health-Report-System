...
def create_one_pager(json_path, output_pdf, logo_path=None):
            ...
            if "issues" in comp_data:
                for issue in comp_data["issues"]:
                    if isinstance(issue, str):  # Handle string issues
                        all_issues.append(
                            {"description": issue, "severity": "medium", "type": "Technical"}
                        )
                    else:
    ...
    doc = SimpleDocTemplate(
        output_pdf, pagesize=letter, rightMargin=40, leftMargin=40, topMargin=40, bottomMargin=40
    )
    story = []
    ...
    subtitle_style = ParagraphStyle(
        "Subtitle", parent=styles["Heading2"], fontSize=14, textColor=HexColor("#334E68")
    )
    score_style = ParagraphStyle(
        "Score", fontSize=28, alignment=1, textColor=HexColor("#102A43"), fontName="Helvetica-Bold"
    )
    ...
    score_data = [
        [
            Paragraph(
                f"<font color='{get_color(overall_score)}'>{overall_score}</font>", score_style
            ),
            Paragraph(f"<font color='{get_color(tech_score)}'>{tech_score}</font>", score_style),
            Paragraph(
                f"<font color='{get_color(content_score)}'>{content_score}</font>", score_style
            ),
    ...
    for item in top_priorities:
        ...
        action_data.append(
            [
                ...
                Paragraph(
                    f"<b>{item.get('type', 'General')}:</b> {clean_desc}<br/><font color='#555'><i>Recommendation: {rec}</i></font>",
                    item_style,
                ),
            ]
        )
    ...
    if not action_data:
        action_data.append(
            [
                Paragraph("✅", styles["Normal"]),
                Paragraph("No critical issues found. Great job!", item_style),
            ]
        )
