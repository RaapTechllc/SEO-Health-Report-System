...
def fetch_page(url: str, timeout: int = 30) -> Optional[str]:
    ...
    try:
        ...
        headers = {"User-Agent": "SEO-Health-Report-Bot/1.0 (AI Visibility Audit)"}
...
def check_semantic_html(html: str) -> Dict[str, Any]:
    ...
    if len(found_semantic) >= 5:
        ...
        findings.append(
            f"Poor: Limited semantic HTML (only: {', '.join(found_semantic) or 'none'})"
        )
        issues.append(
            ParseabilityIssue(
    ...
    if div_count > 0 and semantic_count > 0:
        ...
        if ratio > 20:
            findings.append(
                f"Warning: Heavy div usage ({div_count} divs) - consider more semantic markup"
            )
            issues.append(
                ParseabilityIssue(
                    category="div_soup",
...
def check_content_extraction(html: str) -> Dict[str, Any]:
    ...
    if meta_desc:
        ...
        if 120 <= desc_length <= 160:
            ...
            findings.append(
                f"Fair: Meta description present but suboptimal length ({desc_length} chars)"
            )
            score += 1
    else:
    ...
    if title_match:
        ...
        if 30 <= title_length <= 60:
            ...
            findings.append(
                f"Fair: Title present but length could be optimized ({title_length} chars)"
            )
            score += 1
    else:
    ...
    if html_length > 0:
        ...
        if text_ratio > 0.3:
            ...
            findings.append(
                f"Poor: Low text-to-HTML ratio ({text_ratio:.1%}) - heavy markup or JS-dependent content"
            )
