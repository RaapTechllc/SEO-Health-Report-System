...
### Prerequisites Gate (must complete before coding)

Before any implementation begins, verify:

- [ ] **Dependencies installed?** All required packages/libraries available
- [ ] **Env vars defined?** Required environment variables set
- [ ] **Services running?** DB, auth, cache, queues, etc. accessible
- [ ] **Migrations applied?** Database schema up to date
- [ ] **Tests runnable?** Test suite executes without environment errors

If any are missing, add a **Step 0** task to fix them before implementation.
...
**This section tells `@memory` what to capture when the session ends.**

When implementation is complete, run `@memory` with this context:
...
### What @memory Must Capture
- [ ] Feature shipped: [one-line summary]
- [ ] Key decisions: [list architectural/technical decisions made]
- [ ] Patterns discovered: [any new patterns to add to steering]
- [ ] Risks introduced: [new technical debt, security considerations]
- [ ] Follow-ups: [items deferred or discovered during implementation]

### Steering Updates Needed
- [ ] `tech.md`: [any architecture/stack changes]
- [ ] `structure.md`: [any folder/file convention changes]
- [ ] New steering doc: [if a repeatable rule emerged]
...
- **End-of-session checklist** for `@memory` to consume