...
### Step 3b: Global vs Project-Specific Filter (mandatory)

Before recommending any change, decide if it is:

**A) Global (allowed to patch prompts/steering)**

A fix is Global if:
- It would help in most projects (not tied to this repo's domain)
- It reduces ambiguity in instructions
- It prevents a repeat failure (missing prereqs, unclear outputs, missing steps)
- It standardizes quality gates (tests/typecheck/migrations/security)
- It improves the workflow handshake (plan → execute → review → memory)

**B) Project-Specific (do NOT patch core prompts)**

A fix is Project-Specific if:
- It depends on this repo's business logic or domain rules
- It mentions specific routes/components/tables unique to the project
- It changes stack choices only for this project
- It encodes a workaround for one bug or one environment

**What to do with Project-Specific items:**
- Record it in DEVLOG as a project note
- If it's a repeatable pattern, create a *project-local* doc under:
  `docs/project-notes/` or `.kiro/steering/project-overrides.md`
- Do NOT change master prompts for it
...
After generating this review:
1. Run `@memory` to capture learnings and update documentation
2. Apply recommended steering/command updates

## Memory Handshake

This review provides input for `@memory`. Key items to capture:

- **Process improvements**: [from System Improvement Actions]
- **Patterns to document**: [from Update Steering Documents]
- **Workflow changes**: [from Update Plan/Execute Commands]
- **New automation**: [from Create New Command]
